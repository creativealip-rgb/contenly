...
function normalizeErrorResponse(exception: unknown): { status: number; body: ErrorResponseBody } {
...
export class HttpErrorFilter implements ExceptionFilter {
  private readonly logger = new Logger(HttpErrorFilter.name);
    ...
    if (status >= 500) {
      ...
      this.logger.error(JSON.stringify({ requestId: request.requestId, status, path: request.url, error }));
    }
