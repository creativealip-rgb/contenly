...
=======
  ...
  async generateContent(userId: string, dto: GenerateContentDto) {
    ...
    // Generate content
    console.log(
      `[AiService] Generating content for mode: ${dto.mode}, content length: ${dto.originalContent?.length} `,
    );
    const aiResponse = await this.openAiService.generateContent(
      dto.originalContent,
      {
    ...
    // Convert potential Markdown to HTML as a fallback
    console.log(
      `[AiService] Raw content generated, length: ${content?.length} `,
    );
    content = this.convertMarkdownToHtml(content);
    console.log(
      `[AiService] Content converted to HTML, length: ${content?.length} `,
    );

    // Fetch and inject "Baca Juga" links if we have a site and category
    try {
      console.log(
        `[AiService] Attempting to inject links for user: ${userId}, category: ${dto.categoryId} `,
      );
      ...
      if (sites && sites.length > 0) {
        ...
        console.log(
          `[AiService] Fetched ${recentPosts?.length || 0} recent posts for category ${dto.categoryId}`,
        );

        if (recentPosts && recentPosts.length > 0) {
          content = this.injectInternalLinks(content, recentPosts);
          console.log(
            `[AiService] Successfully injected ${recentPosts.length} links`,
          );
        } else {
          console.warn(
            `[AiService] No recent posts found for category ${dto.categoryId}`,
          );
        }
      }
    } catch (linkError) {
...
>>>>>>> c1209c3 (feat: global styling refresh, layout fixes, and build error resolution)
  ...
  private injectInternalLinks(
    ...
    console.log(
      `[AiService] Injecting 3 links into ${isHtml ? 'HTML' : 'Markdown'} (${actualChunks.length} paragraphs)`,
    );
