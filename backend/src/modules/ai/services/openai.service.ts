...
- slug: URL-friendly slug`;
  ...
  async generateImage(prompt: string): Promise<string> {
    // Primary: Use Pollinations.ai (free, no API key required)
    console.log(`🎨 Generating image via Pollinations.ai for: ${prompt.substring(0, 80)}...`);
    try {
      const encodedPrompt = encodeURIComponent(prompt);
      const imageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=1024&height=1024&nologo=true`;
      
      // Verify the image was generated by checking response
      const response = await fetch(imageUrl, { method: 'HEAD', signal: AbortSignal.timeout(60000) });
      if (response.ok && response.headers.get('content-type')?.startsWith('image/')) {
        console.log(`✅ Image generated via Pollinations.ai: ${imageUrl.substring(0, 80)}...`);
        return imageUrl;
      }
      console.warn(`⚠️ Pollinations.ai returned ${response.status}, falling back to DALL-E`);
    } catch (err: any) {
      console.warn(`⚠️ Pollinations.ai failed: ${err.message}, falling back to DALL-E`);
    }
    ...
    if (this.nativeOpenai) {
      try {
        ...
        console.warn(`⚠️ DALL-E fallback also failed: ${err.message}`);
      }
    }
