...
export class AuthService {
  ...
  async signUp(data: { email: string; password: string; name?: string }) {
    try {
      ...
      if (result.user) {
        ...
        this.sendVerificationEmail(data.email).catch(err => 
  ...
  async sendVerificationEmail(email: string) {
    try {
      ...
      const verifyUrl = `${process.env.FRONTEND_URL?.split(',')[0] || 'http://localhost:3000'}/verify-email?token=${token}`;
      await this.sendVerificationEmailLink(email, verifyUrl);
  ...
  async verifyEmail(token: string) {
    try {
      ...
      const email = validToken.identifier.replace('verify:', '');
  ...
  private async sendVerificationEmailLink(email: string, verifyUrl: string) {
    const emailFrom = this.configService.get<string>('EMAIL_FROM') || 'noreply@contenly.app';
    ...
    if (this.resend) {
      try {
        await this.resend.emails.send({
          from: emailFrom,
          to: email,
          subject: 'Verifikasi Email Anda - Contenly',
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2 style="color: #2563eb;">Verifikasi Email Anda</h2>
              <p>Terima kasih telah mendaftar di Contenly!</p>
              <p>Klik tombol di bawah ini untuk memverifikasi email Anda:</p>
              <a href="${verifyUrl}" style="display: inline-block; background: linear-gradient(to right, #2563eb, #4f46e5); color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; margin: 16px 0;">Verifikasi Email</a>
              <p style="color: #6b7280; font-size: 14px;">Link ini akan kedaluwarsa dalam 24 jam.</p>
              <p style="color: #6b7280; font-size: 14px;">Jika Anda tidak mendaftar di Contenly, abaikan email ini.</p>
            </div>
          `,
        });
