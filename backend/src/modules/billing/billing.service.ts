...
export class BillingService {
  ...
  async getBalance(userId: string) {
    ...
    if (!balance) {
      // Create initial balance (handle concurrent inserts)
      try {
        ...
        balance = await this.db.query.tokenBalance.findFirst({
          where: eq(tokenBalance.userId, userId),
        });
      }
    }
  ...
  async checkMonthlyLimit(userId: string, featureType: string): Promise<boolean> {
  ...
  async checkCategoryLimit(userId: string, featureType: string): Promise<boolean> {
    ...
    return this.checkMonthlyLimit(userId, categoryFeature);
  }
  ...
  async recordUsage(userId: string, featureType: string, billingResult?: { usingKredit?: boolean; kreditCost?: number }) {
    ...
    if (billingResult?.usingKredit && (billingResult.kreditCost || 0) > 0) {
      ...
      // Record monthly quota usage in transaction history
      const categoryLabels: Record<string, string> = { ARTICLE_GENERATION: 'Pembuatan artikel', INSTAGRAM_GENERATION: 'IG Carousel', VIDEO_LIGHT: 'Video', VIDEO_HEAVY: 'Video Heavy', IMAGE_GENERATION: 'Generate gambar', MOTION_RENDER: 'Motion render' };
      const catLabel = categoryLabels[category] || featureType;
      await this.db.insert(transaction).values({
        ...
        tokens: 0,
        status: 'COMPLETED',
        metadata: { description: featureType, deductedFrom: 'monthly', categoryLabel: catLabel },
      });
    }
  }
  ...
  async addTokens(userId: string, amount: number, stripePaymentId?: string, isReset: boolean = false, paymentAmount?: number) {
    await this.db.transaction(async (tx) => {
  ...
  async handleInvoicePaymentSucceeded(invoice: Stripe.Invoice) {
    ...
    // Get subscription to find userId and tokens
    const sub = await this.db.query.subscription.findFirst({
      where: eq(subscription.stripeSubscriptionId, subscriptionId as string),
    });

    const userId = invoice.metadata?.userId || sub?.userId;
    if (!userId) {
      this.logger.error('No userId found in invoice metadata or subscription');
      return;
    }
