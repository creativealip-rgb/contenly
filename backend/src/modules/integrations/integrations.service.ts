...
export class IntegrationsService {
  ...
  async updateSite(siteId: string, userId: string, dto: UpdateSiteDto) {
    ...
    if (dto.url)
      updates.url = dto.url.endsWith('/') ? dto.url.slice(0, -1) : dto.url;
    if (dto.username) updates.username = dto.username;
    if (dto.appPassword)
      updates.appPasswordEncrypted = this.encryptPassword(dto.appPassword);
  ...
  async testConnection(siteId: string, userId: string) {
    ...
    try {
      const credentials = Buffer.from(
        `${site.username}:${decryptedPassword}`,
      ).toString('base64');
  ...
  async refreshCategories(siteId: string, userId: string) {
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to refresh categories: ${error.message}`,
      );
    }
  }
