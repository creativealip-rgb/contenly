...
${project.sourceContent}`;
  ...
  async exportVideo(
    ...
    const outputPath = path.join(outputDir, `${project.id}-${Date.now()}.mp4`);
    const audioPath = path.join(outputDir, `${project.id}-${Date.now()}.mp3`);
    const subtitlePath = path.join(outputDir, `${project.id}-${Date.now()}.srt`);
    ...
    const filter = [
      `scale=${width}:${height}`,
      `drawtext=text='${safeTitle}':fontcolor=white:fontsize=64:x=(w-text_w)/2:y=220:box=1:boxcolor=black@0.35:boxborderw=24`,
      `drawtext=text='${safeSub}':fontcolor=white:fontsize=38:x=(w-text_w)/2:y=340:box=1:boxcolor=black@0.25:boxborderw=18`,
    ...
    await execFileAsync('ffmpeg', [
      '-y',
      '-f', 'lavfi',
      '-i', `color=c=#111827:s=${width}x${height}:d=${Math.max(5, totalDuration)}`,
      '-i', audioPath,
      '-vf', filter,
      '-c:v', 'libx264',
      '-preset', 'veryfast',
    ...
    await fs.unlink(subtitlePath).catch(() => undefined);
