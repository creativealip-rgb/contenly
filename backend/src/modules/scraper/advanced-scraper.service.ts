...
export class AdvancedScraperService {
    ...
    async scrapeArticle(url: string): Promise<AdvancedScrapedContent> {
        try {
            ...
            this.logger.error(`Failed to scrape ${url}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            throw new BadRequestException(`Failed to scrape article: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
     ...
     * Success Rate: ~95%
     */
    private async extractWithReadability(url: string, html: string): Promise<Omit<AdvancedScrapedContent, 'extractionTier'> | null> {
        try {
            ...
            this.logger.warn(`Tier 1 (Readability) failed: ${error instanceof Error ? error.message : 'Unknown'}`);
            return null;
        }
    }
     ...
     * Success Rate: ~80%
     */
    private async extractWithHeuristics(html: string, url: string): Promise<Omit<AdvancedScrapedContent, 'extractionTier'> | null> {
        try {
            const $ = cheerio.load(html);

            // Remove noise
            $('script, style, nav, footer, header, aside, .advertisement, .ad, .sidebar, .comments, .related-posts').remove();
            ...
            this.logger.warn(`Tier 2 (Heuristics) failed: ${error instanceof Error ? error.message : 'Unknown'}`);
            return null;
        }
    }
     ...
     * Success Rate: ~50%
     */
    private async extractWithSelectors(html: string, url: string): Promise<Omit<AdvancedScrapedContent, 'extractionTier'> | null> {
        try {
            const $ = cheerio.load(html);
            ...
            this.logger.warn(`Tier 3 (Selectors) failed: ${error instanceof Error ? error.message : 'Unknown'}`);
            return null;
        }
    }
     ...
     */
    private calculateContentScore($elem: cheerio.Cheerio<any>, text: string): number {
        let score = 0;
     ...
     */
    private extractImages($elem: cheerio.Cheerio<any>, baseUrl: string): string[] {
        const images: string[] = [];
