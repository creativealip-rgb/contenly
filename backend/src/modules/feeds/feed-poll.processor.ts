...
export class FeedPollProcessor {
    ...
    async handlePollFeed(job: Job<{ feedId: string }>) {
        ...
        try {
            ...
            this.logger.log(`Successfully polled feed ${feedId}: ${result.newItems} new items, ${result.totalItems} total`);
            return result;
        } catch (error) {
    ...
    async handlePollAllFeeds(job: Job) {
        ...
        try {
            ...
            this.logger.error(`Failed to poll all feeds: ${error instanceof Error ? error.message : 'Unknown error'}`);
            throw error;
        }
    }
}
