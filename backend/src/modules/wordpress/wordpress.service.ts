...
export class WordpressService implements OnModuleInit {
  ...
  async publishArticle(
    ...
    try {
      ...
      // Build reliable post URL — WP sometimes returns /blog or ?p=ID for drafts
      const postLink = response.data.link && !response.data.link.endsWith('/blog')
        ? response.data.link
        : `${site.url}/${response.data.slug}`;
