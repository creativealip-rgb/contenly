...
export class WordpressService {
  ...
  constructor(
    ...
    this.encryptionKey =
      ...
      'default-encryption-key-32-bytes!!';
  }
  ...
  async syncCategories(siteId: string) {
    ...
    console.log('[syncCategories] Site found:', {
      name: site.name,
      url: site.url,
    });
    ...
    try {
      ...
      console.log(
        '[syncCategories] Calling WordPress API:',
      ...
      console.log(
        '[syncCategories] API response received:',
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          console.log(
            '[publishArticle] Featured image uploaded, ID:',
            featuredMediaId,
          );
        } catch (imgError) {
          console.error(
            '[publishArticle] Failed to upload featured image, continuing without it:',
            imgError,
          );
        }
      }
      ...
      try {
        ...
        if (!articleId) {
          const existing = await this.findExistingArticle(
            userId,
            dto.feedItemId,
            dto.sourceUrl,
            dto.title,
          ...
          if (existing) {
            ...
            console.log(
              '[publishArticle] Found existing article by match:',
              articleId,
              'Current status:',
              existing.status,
            );
        ...
        if (articleId) {
          await this.articlesService.update(userId, articleId, articleData);
          console.log(
            '[publishArticle] Updated existing article data:',
            articleId,
          );

          await this.articlesService.updateStatus(articleId, articleStatus, {
            wpPostId: String(response.data.id),
            wpPostUrl: response.data.link,
          });
          console.log(
            '[publishArticle] Updated article status to:',
            articleStatus,
          );
        } else {
          ...
          console.log(
            '[publishArticle] Created new article with status:',
            articleStatus,
          );
        }
      } catch (dbError) {
        console.error(
          '[publishArticle] Failed to save/update local DB:',
          dbError,
        );
        throw new BadRequestException(
          'Failed to save article to database: ' + dbError.message,
        );
      }
  ...
  private async findExistingArticle(
    ...
    if (!feedItemId && !sourceUrl && !title) {
      return null;
    }

    const conditions = [
      eq(article.userId, userId),
      inArray(article.status, ['GENERATED', 'DRAFT']),
      isNull(article.wpPostId),
    ];

    if (feedItemId) {
      conditions.push(eq(article.feedItemId, feedItemId));
    } else if (sourceUrl) {
      conditions.push(eq(article.sourceUrl, sourceUrl));
    } else if (title) {
      conditions.push(eq(article.title, title));
    }
  ...
  async getRecentPosts(
    ...
    try {
      ...
      if (posts.length === 0 && categoryId) {
        console.log(
          `[WordpressService] No posts in category ${categoryId}, falling back to recent posts`,
        );
