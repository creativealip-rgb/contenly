...
export class AnalyticsService {
  ...
  async getDashboardStats(userId: string) {
    ...
    const activity = [
      ...
      ...recentTransactions.map((t) => {
        ...
        const amount = Math.abs(t.tokens ?? 0);
        return {
          id: t.id,
          // Was hardcoded 'tokens_purchased' for every row, so token USAGE
          // showed up labelled/iconed as a purchase with a negative count.
          type: isPurchase ? 'tokens_purchased' : 'tokens_used',
          title: isPurchase ? `+${amount} Token` : `-${amount} Token`,
          description: isPurchase ? 'Token dibeli' : 'Token terpakai',
  ...
  async getTokenUsage(userId: string, days = 30) {
    ...
    const grouped = transactions.reduce((acc, t) => {
      ...
      // USAGE transactions store token deltas as negative debits; show consumption as a positive figure
      acc[date] += Math.abs(t.tokens ?? 0);
      return acc;
  ...
  async getAdminStats() {
    ...
    const [activeUsers] = await this.db
      .select({ count: sql<number>`count(distinct ${transaction.userId})` })
      .from(transaction)
      .where(gte(transaction.createdAt, thirtyDaysAgo));

    // Get total revenue
    const [totalRevenue] = await this.db
      .select({ total: sql<number>`coalesce(sum(${transaction.amount}), 0)` })
      .from(transaction)
      .where(eq(transaction.type, 'PURCHASE'));
    ...
    const [totalVideos] = await this.db
      .select({ count: sql<number>`count(*)` })
      .from(scriptProject);
    ...
    plans.forEach((p) => {
      planDistribution[p.plan] = p.count;
    });
