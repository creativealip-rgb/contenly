import { pgTable, uuid, varchar, text, boolean, integer, real, timestamp, pgEnum, jsonb, primaryKey, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const articleStatusEnum = pgEnum('article_status', ['DRAFT', 'GENERATING', 'READY', 'PUBLISHING', 'PUBLISHED', 'FAILED']);
...
export const categoryMapping = pgTable('category_mapping', {
    id: uuid('id').primaryKey().defaultRandom(),
    wpSiteId: uuid('wp_site_id').notNull().references(() => wpSite.id, { onDelete: 'cascade' }),
    sourceCategory: varchar('source_category', { length: 255 }).notNull(),
    targetCategoryId: varchar('target_category_id', { length: 50 }).notNull(),
    targetCategoryName: varchar('target_category_name', { length: 255 }).notNull(),
}, (table) => ({
    uniqueMapping: uniqueIndex('unique_wp_site_source_category').on(table.wpSiteId, table.sourceCategory),
}));
...
export const feed = pgTable('feed', {
    ...
    defaultWpSiteId: uuid('default_wp_site_id').references(() => wpSite.id, { onDelete: 'set null' }),
...
export const article = pgTable('article', {
    id: uuid('id').primaryKey().defaultRandom(),
    userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
    feedItemId: uuid('feed_item_id').references(() => feedItem.id, { onDelete: 'set null' }),
    wpSiteId: uuid('wp_site_id').references(() => wpSite.id, { onDelete: 'set null' }),
...
export const feedRelations = relations(feed, ({ one, many }) => ({
    ...
    defaultWpSite: one(wpSite, { fields: [feed.defaultWpSiteId], references: [wpSite.id] }),
    items: many(feedItem),
}));

export const feedItemRelations = relations(feedItem, ({ one }) => ({
    feed: one(feed, { fields: [feedItem.feedId], references: [feed.id] }),
    article: one(article, { fields: [feedItem.id], references: [article.feedItemId] }),
}));
