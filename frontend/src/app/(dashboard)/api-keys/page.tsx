...
import { Key, Save, Trash2, Eye, EyeOff, Loader2, CheckCircle2, XCircle, ExternalLink, TestTube2, MessageSquare, ImageIcon } from 'lucide-react'
...
export default function ApiKeysPage() {
    ...
    async function loadKeys() {
        try {
            const settings = await api.get<any[]>('/admin/settings?category=api-keys')
    ...
    async function testFeature(type: 'chat' | 'image', provider: string) {
        ...
        try {
            ...
            const result = await api.post(endpoint, {
                provider,
                model,
                prompt: type === 'chat' ? testPrompt : imagePrompt,
            })
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
            ...
            <Tabs defaultValue="keys">
                                ...
                                <CardContent>
                                    <div className="flex gap-2">
                                        <div className="flex-1 relative">
                                            ...
                                            />
                                            <Button
                                                variant="ghost" size="sm" className="absolute right-1 top-1/2 -translate-y-1/2 h-7 w-7 p-0"
                                        ...
                                        <Button
                                            onClick={() => saveKey(provider.key, keys[provider.key])}
                ...
                <TabsContent value="test" className="space-y-4">
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">Test Chat Completion</CardTitle>
                        </CardHeader>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {PROVIDERS.filter(p => keys[p.key]).map((provider) => (
                                    <div key={provider.key} className="flex gap-1">
                                        ...
                                        <Button
                                            variant="outline"
                                            onClick={() => testFeature('chat', provider.key)}
                                            disabled={testing === `chat-${provider.key}`}
                                        >
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">Test Image Generation</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            <Input value={imagePrompt} onChange={(e) => setImagePrompt(e.target.value)} placeholder="Image prompt..." />
                            <div className="flex flex-wrap gap-2">
                                {PROVIDERS.filter(p => keys[p.key] && p.models.some(m => m.includes('dall') || m.includes('image') || m.includes('flash'))).map((provider) => (
                                    <Button
                                        key={provider.key}
                                        variant="outline"
                                        onClick={() => testFeature('image', provider.key)}
                                        disabled={testing === `image-${provider.key}`}
                                    >
                        ...
                        <Card className={testResult.success ? 'border-green-500' : 'border-red-500'}>
                            ...
                            <CardContent>
                                {testResult.success && testResult.response && (
                                    <p className="text-sm bg-muted p-3 rounded">{testResult.response}</p>
                                )}
                                {testResult.success && testResult.imageUrl && (
                                    <img src={testResult.imageUrl} alt="Generated" className="max-w-md rounded" />
                                )}
                                {!testResult.success && (
